---
title: "백엔드 개발자의 프론트 도전기 — AI + 디자인 시스템 + Tailwind로 버티기"
date: 2026-06-11
...
프론트 폴더 구조는 우리 팀에서 이미 만들어둔 **사내 프론트엔드 스캐폴딩**을 기반으로 세팅했다. React Router v7 기반에 FSD(Feature-Sliced Design) 아키텍처가 잡혀있는 스캐폴딩이다.
...
<div style={{background: '#0d1117', border: '1px solid #30363d', borderRadius: '12px', padding: '1.5rem', margin: '1.5rem 0'}}>
  ...
  <div style={{display: 'flex', alignItems: 'center', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap'}}>
    {[
      {step: '1', label: '디자인 시스템', sub: '사내 공통 컴포넌트', color: '#a78bfa', bg: '#1a1530'},
...
디자인 시스템에 없는 패턴이 반복되면 `shared/ui`로 올렸다. 실제로 만들어진 컴포넌트들을 보면:
...
예를 들어 `Pagination`은 디자인 시스템에 `PaginationWrapper`가 있는데 반응형 처리가 필요해서 한 번 감싸뒀고, `SearchBar`는 검색 인풋과 버튼을 묶은 우리 서비스 패턴이라 직접 만들었다.
