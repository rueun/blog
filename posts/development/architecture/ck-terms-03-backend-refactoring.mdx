...
> 이 글은 CK Terms 서비스 전면 리팩토링 과정을 기록한 시리즈 세 번째 글입니다.

---

## 기존 구조가 문제였던 이유

기존 CK Terms 백엔드는 Maven 단일 모듈이었다. 구조를 보면:
...
```groovy
rootProject.name = 'ck-terms-backend'

include(
        ':application:terms-api',   // REST API 진입점
...
<div style={{background: '#0d1117', border: '1px solid #30363d', borderRadius: '12px', padding: '1.5rem', margin: '1.5rem 0'}}>
  ...
  <div style={{display: 'flex', alignItems: 'stretch', gap: '0.6rem', justifyContent: 'center', flexWrap: 'wrap'}}>
    {[
      {name: 'application', sub: 'terms-api', color: '#3b82f6', bg: '#0f1729'},
